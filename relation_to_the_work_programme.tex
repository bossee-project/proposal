...
\TODO{Check claim, Thomas Kluyver?}
\begin{itemize}
...
% Often, the Jupyter notebook is run 'locally' so that the Jupyter
% kernel and Jupyter server is running on the same machine as the web
% browser that acts as the front-end and hosts the 'notebook' that the
% users see. However, increasingly, institutions start to deploy
% so-called JupyterHub services, that allow to host the Jupyter Server
% and kernel on a different machine (and potentially in the cloud) from
% the user's machine, on which the user views the notebook in their
% browser.


These projects are building upon Jupyter. A particular example is the
