\begin{task}[
  ...
  The OpenDose project \TODO{Can we add a URL to the project, or some
    other reference?}
  ...
  The idea is to impulse \TODO{``impulse'' sounds odd. Do you mean
    ``trigger''?} the collaborative development of a reference database,
  ...
  \TODO{What is CRCT?},
  processed and fed into a local \TODO{local at CRCT or at your site?} SQL database.

  This collaborative effort rise \TODO{do you mean ``raises'' instead
    of ``rise''?} some challenges:
  \begin{compactitem}
  \item Data production: a total of 750 K hours of CPU time is needed per
    model. \TODO{Do you mean 750,000 hours of CPU time per model?
      Maybe use ``kilo hours'' instead of ``K hours''? Or at least ``k
      hours''.}
  ...
  \item Developing tools to work seamlessly on the SQL database
    \TODO{Which keeps which data? Best to remind the reader about the
      application here.}
  \item Developing data analysis tools. \TODO{`'using the Python data
      science ecosystem where possible''.}
  \item Developing visualization tools. \TODO{``exploring Widgets
      inside the Notebook for interactivity''?}
